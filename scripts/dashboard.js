...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderSummary() {
    ...
    if (s.overdueCount > 0) {
      document.getElementById('stat-overdue').textContent = '⚠️ ' + s.overdueCount + ' прострочено';
      document.getElementById('stat-overdue').style.color = 'var(--danger)';
    } else {
      document.getElementById('stat-overdue').textContent = 'Прострочень немає';
    }
  }
  ...
  function renderLoans() {
    ...
    grid.querySelectorAll('.btn-del').forEach(btn => {
      btn.addEventListener('click', () => {
        if (confirm('Видалити кредит "' + DB.getById(btn.dataset.id)?.name + '"?')) {
  ...
  function loanCardHTML(loan) {
    ...
    const pct = loan.originalAmount > 0
      ? ((loan.originalAmount - loan.currentBalance) / loan.originalAmount * 100).toFixed(1)
      : 0;
    const days = Calc.daysUntil(loan.nextPaymentDate);
    const daysLabel = days < 0 ? `прострочено ${Math.abs(days)} дн.` : days === 0 ? 'сьогодні!' : `через ${days} дн.`;
    ...
    return `<div class="loan-card ${isOverdue ? 'overdue' : ''}">
      ...
      <div class="progress-wrap">
        ...
        <div class="progress-labels">
          ...
          <span>${Calc.daysUntil(loan.nextPaymentDate) !== null ? daysLabel : ''}</span>
        </div>
      </div>
      <div class="card-details">
        <div><div class="detail-label">Щомісяця</div><div class="detail-val">${Calc.formatMoney(loan.monthlyPayment)}</div></div>
        <div><div class="detail-label">Ставка</div><div class="detail-val">${loan.rate}% річних</div></div>
  ...
  function renderUpcoming() {
    ...
    const upcoming = DB.upcomingPayments(4);
    if (!upcoming.length) { list.innerHTML = '<div style="color:var(--muted);font-size:13px;padding:12px 0">Немає кредитів</div>'; return; }
  ...
  document.getElementById('pay-modal')?.addEventListener('click', e => {
    if (e.target.id === 'pay-modal') document.getElementById('pay-modal').classList.remove('open');
  });
